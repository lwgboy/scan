...
public class MoreHandleActivity extends Activity implements OnClickListener {
	...
	this.editKillPassword = (EditText) findViewById(
		R.id.edit_kill_password);
	...
	this.adatpterMemBank.setDropDownViewResource(
		android.R.layout.simple_spinner_dropdown_item);
	this.spinnerLockType = (Spinner) findViewById(R.id.spinner_lock_type);
	this.adapterLockType = ArrayAdapter.createFromResource(this,
		R.array.arr_lockType, android.R.layout.simple_spinner_item);
	this.adapterLockType.setDropDownViewResource(
		android.R.layout.simple_spinner_dropdown_item);
	...
	byte[] accessPassword = Tools
		.HexString2Bytes(editPassword.getText().toString());
	...
	case R.id.button_read:
		...
		Toast.makeText(getApplicationContext(), "密码为4个字节",
			Toast.LENGTH_SHORT).show();
		return;
	    }
	    // 读取数据区数据
	    byte[] data = reader.readFrom6C(membank, addr, length,
		    accessPassword);
	...
	case R.id.button_write:
		...
		Toast.makeText(getApplicationContext(),
			"写入数据的长度以字为单位，1word = 2bytes", Toast.LENGTH_SHORT)
			.show();
	    }
	    byte[] dataBytes = Tools.HexString2Bytes(writeData);
	    // dataLen = dataBytes/2 dataLen是以字为单位的
	    boolean writeFlag = reader.writeTo6C(accessPassword, membank, addr,
		    dataBytes.length / 2, dataBytes);
	    if (writeFlag) {
